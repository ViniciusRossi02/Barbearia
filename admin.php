...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $acao  = $_POST['acao'] ?? '';
  $id    = (int) ($_POST['id'] ?? 0);
  $valor = isset($_POST['valor']) && $_POST['valor'] !== '' ? (float) str_replace(',', '.', $_POST['valor']) : null;

  if ($acao === 'confirmar' && $id > 0) {
    confirmar_agendamento($id, $valor);
    $feedback = 'Agendamento confirmado' . ($valor ? ' — R$ ' . number_format($valor, 2, ',', '.') . ' registrado!' : '!');
  } elseif ($acao === 'recusar' && $id > 0) {
    recusar_agendamento($id);
    $feedback = 'Agendamento recusado.';
  } elseif ($acao === 'atualizar_valor' && $id > 0) {
    if (agendamento_e_passado($id)) {
      $feedback = '⚠️ Não é possível editar agendamentos passados.';
    } else {
      atualizar_valor($id, $valor);
      $feedback = 'Valor atualizado' . ($valor ? ' para R$ ' . number_format($valor, 2, ',', '.') . '!' : ' — removido.');
    }
  } elseif ($acao === 'desconfirmar' && $id > 0) {
    if (agendamento_e_passado($id)) {
      $feedback = '⚠️ Não é possível alterar agendamentos passados.';
    } else {
      desconfirmar_agendamento($id);
      $feedback = 'Agendamento devolvido para pendente.';
    }
  }
}
...
$proximos_dias  = listar_proximos_dias(30);
$total_hoje     = contar_agendamentos_hoje();
$total_pendente = contar_pendentes();
$total_semana   = contar_agendamentos_semana();
$fat_hoje       = faturamento_do_dia(date('Y-m-d'));
$fat_semana     = faturamento_da_semana();
$fat_mes        = faturamento_do_mes();
...
<head>
  ...
  <style>
    ...
    *::after {
    ...
    .header-links a.destaque {
      border-color: var(--verde);
      color: var(--verde);
    }

    .header-links a.destaque:hover {
    ...
    .dia-card.ativo .dc-num {
      color: var(--verde);
    }

    .dia-card.ativo .dc-total {
      background: #1a2a00;
      color: #b8f040;
    }
...
<body>
  ...
  <main>
    ...
    <div class="fat-grid">
      <div class="fat-card">
        <div class="fat-valor">R$ <?= number_format($fat_hoje, 2, ',', '.') ?></div>
        <div class="fat-rotulo">Faturado hoje</div>
      </div>
      <div class="fat-card">
        <div class="fat-valor">R$ <?= number_format($fat_semana, 2, ',', '.') ?></div>
        <div class="fat-rotulo">Faturado na semana</div>
      </div>
      <div class="fat-card">
        <div class="fat-valor">R$ <?= number_format($fat_mes, 2, ',', '.') ?></div>
        <div class="fat-rotulo">Faturado no mês</div>
      </div>
    </div>
          ...
          <tbody>
            <?php foreach ($agenda_hoje as $a): ?>
              <tr>
                ...
                <td style="color:var(--verde);font-weight:600;font-size:.85rem">
                  <?= $a['valor'] !== null ? 'R$ ' . number_format((float)$a['valor'], 2, ',', '.') : '—' ?>
                </td>
                ...
                <td>
                  <div class="acoes">
                    ...
                    <form method="POST" style="margin:0;display:flex;gap:6px;align-items:center">
                      ...
                      <input type="number" name="valor" class="input-valor" step="0.01" min="0"
                        placeholder="Novo valor"
                        value="<?= $a['valor'] !== null ? number_format((float)$a['valor'], 2, '.', '') : '' ?>">
                      <button type="submit" class="btn-acao btn-editar-valor" title="Salvar novo valor">✎ valor</button>
                    </form>
                    <!-- Desconfirmar -->
                    <form method="POST" style="margin:0"
                      onsubmit="return confirm('Devolver para pendente?')">
                      <input type="hidden" name="acao" value="desconfirmar">
                      <input type="hidden" name="id" value="<?= $a['id'] ?>">
                      <button type="submit" class="btn-acao btn-recusar" title="Desconfirmar">↩ desfazer</button>
                    </form>
                  </div>
                </td>
              </tr>
                          ...
                          <div class="acoes">
                            <form method="POST" style="margin:0;display:flex;gap:6px;align-items:center">
                              ...
                              <input type="number" name="valor" class="input-valor"
                                placeholder="R$ valor" step="0.01" min="0">
